...
create function throw_no_permission(_tenant_id int, _user_id bigint, _perm_codes text[])
    returns void
    language plpgsql
as
$$
...
$$
begin
    perform throw_no_permission(_tenant_id, _user_id, array [_perm_code]);
end;
$$;

create or replace function has_permissions(_tenant_id int, _user_id bigint, _perm_codes text[],
...
begin
    ...
    if (_throw_err) then
        ...
        perform throw_no_permission(_tenant_id, _user_id, _perm_codes);
    end if;
...
begin
    perform has_permission(1, _user_id, 'system.manage_tenants.create_tenant');
...
declare
    __ug_id int;
begin
    perform has_permission(1, _user_id, 'system.manage_tenants.assign_owner');
...
$$
begin
    perform has_permission(_tenant_id, _user_id, 'system.manage_groups.update_group');
...
create function public.delete_user_group(_user_id bigint, _deleted_by text, _tenant_id int, _ug_id int)
    returns table
            (
                __group_id int
            )
...
$$
begin
    perform has_permission(_tenant_id, _user_id, 'system.manage_groups.delete_group');
...
create function public.delete_user_group_member(_user_id bigint, _deleted_by text, _tenant_id int, _ug_id int,
                                                _user_member_id int)
    returns void
    language plpgsql
as
...
begin
    perform has_permission(_tenant_id, _user_id, 'system.manage_groups.delete_member');
...
create function public.add_user_group_mapping(_user_id bigint, _created_by text, _tenant_id int, _ug_id int,
...
$$
begin
    perform has_permission(_tenant_id, _user_id, 'system.manage_groups.add_mapping');

    return query
        insert into user_group_mapping (created_by, group_id, mapped_object_id, mapped_object_name)
            values (_created_by, _ug_id, _mapped_object_id, _mapped_object_name)
...
create function public.delete_user_group_mapping(_user_id bigint, _deleted_by text, _tenant_id int, _ug_id int,
                                                 _ug_mapping_id int)
    returns void
    language plpgsql
as
...
-- create or replace function has_permission(_tenant_code text, _user_id bigint, _perm_code text,
--                                           _throw_err bool default true)
-- 	returns bool
-- 	language plpgsql
-- 	stable
...
-- 	into __tenant_id;
--
-- 	return has_permission(__tenant_id, _user_id, _perm_code, _throw_err);
-- end;
-- $$;
...
begin
    ...
    select array_agg(distinct groups.code)                      rs,
...
create function auth.add_user_to_group(_created_by text, _user_id bigint, _target_user_id bigint, _group_id int)
    returns setof user_group_member
    language plpgsql
    rows 1
as
...
begin
    ...
    select tenant_id, is_system, is_assignable, is_active
    from user_group ug
    where ug.user_group_id = _group_id
    ...
    perform has_permission(__tenant_id, _user_id, 'system.manage_groups.add_member');

    return query
        insert into user_group_member (created_by, group_id, user_id)
            values (_created_by, _group_id, _target_user_id)
            returning *;
...
create or replace function public.ensure_roles_and_permissions(_oid text, _provider_groups text[])
    returns table
            (
                __roles       text[],
                __permissions text[]
...
begin
    ...
    perform unsecure.create_permission_by_path_as_system('Add member', 'system.manage_groups');
    perform unsecure.create_permission_by_path_as_system('Delete member', 'system.manage_groups');

    perform unsecure.create_permission_by_path_as_system('Add mapping', 'system.manage_groups');
