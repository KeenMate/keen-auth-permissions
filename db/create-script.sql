...
create unique index uq_user_info on user_info (email, oid);
...
create trigger c_perm_set_code
    ...
    on perm_set
    for each row
execute procedure trg_generate_code_from_title();
...
as
$$
insert into user_info(created_by, modified_by, can_login, email, display_name, username, oid, password_hash)
values ('initial_script', 'initial_script', false, 'system', 'System', 'system', ext.uuid_generate_v4()::text, '')
returning *;

$$;

create function unsecure.create_user_as_system(_email text, _display_name text, _username text, _oid text,
                                               _password_hash text,
...
$$
insert into user_info (created_by, modified_by, can_login, email, display_name, username, oid, password_hash)
values ('initial_script', 'initial_script', _can_login, _email, _display_name, _username, _oid, _password_hash)
returning *;
$$;
...
-- PROBABLY NOT USED AND CAN BE REMOVED, there is no _oid anyway
create or replace function public.get_roles_and_permissions(_oid text)
    returns table
            (
                __roles       text[],
                __permissions text[]
            )
    returns null on null input ROWS 1
    language sql
    stable
as
$$
...
create function ensure_user_from_provider(_created_by text, _provider text, _provider_uid text, _user_code text default null, _username text default )
    returns table
            (
                __user_id      bigint,
                __code         text,
...
begin
    ...
    if not exists(select
                  ...
                  where ui.username = lower(_username)) then
...
create function ensure_user_data(_created_by text, _username text, _provider text, _user_data jsonb)
    returns table
            (
                __user_id      bigint,
