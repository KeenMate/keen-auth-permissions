...
create function public.create_user_group(_user_id bigint, _created_by text, _tenant_id int, _title text, _is_assignable bool, _is_active bool)
	returns table
	        (
		        __group_id int
	        )
...
begin
	...
	return query
		insert into user_group (created_by, modified_by, tenant_id, title, is_default, is_system, is_assignable, is_active)
			values (_created_by, _created_by, _tenant_id, _title, false, false, _is_assignable, _is_active)
			returning user_group_id;
end;
$$;

create function public.update_user_group(_user_id bigint, _modified_by text, _tenant_id int, _ug_id int, _title text, _is_assignable bool, _is_active bool)
	returns table
	        (
		        __group_id      int,
		        __title         text,
...
create function public.add_user_group_member(_user_id bigint, _created_by text, _tenant_id int, _ug_id int, _user_member_id int)
	returns table
	        (
...
begin
	...
	values (_created_by, _ug_id, _user_member_id, true)
	returning member_id;
end;
$$;

create function public.delete_user_group_member(_user_id bigint, _created_by text, _tenant_id int, _ug_id int, _user_member_id int)
	returns void
	language plpgsql
as
$$
...
create or replace function public.register_user(_tenant_id int, _username text, _password_hash text, _email text, _display_name text, _user_data text)
	returns table
	        (
		        __user_id      bigint,
		        __code         text,
...
begin
	...
	perform unsecure.create_permission_by_path_as_system('Manage groups');
