...
defmodule KeenAuthPermissions.Permissions do
  ...
  @doc """
  Throws a no permission error.

  Calls `auth.throw_no_permission`.
  There are multiple overloads - this uses the (user_id, perm_code) variant.
  """
  @spec throw_no_permission(integer(), String.t()) :: {:error, any()}
  def throw_no_permission(user_id, perm_code) when is_binary(perm_code) do
    db_context().auth_throw_no_permission(user_id, perm_code)
    |> ErrorParsers.parse_if_error()
  end

  @doc """
  Throws a no permission error with tenant.

  Calls `auth.throw_no_permission`.
  Uses the (user_id, perm_code, tenant_id) variant.
  """
  @spec throw_no_permission(integer(), String.t(), integer()) :: {:error, any()}
  def throw_no_permission(user_id, perm_code, tenant_id) when is_binary(perm_code) do
    db_context().auth_throw_no_permission(user_id, perm_code, tenant_id)
    |> ErrorParsers.parse_if_error()
  end

  @doc """
  Throws a no permission error for multiple permission codes.

  Calls `auth.throw_no_permission`.
  Uses the (user_id, perm_codes) variant with a list.
  """
  @spec throw_no_permission_list(integer(), list(String.t())) :: {:error, any()}
  def throw_no_permission_list(user_id, perm_codes) when is_list(perm_codes) do
    db_context().auth_throw_no_permission(user_id, perm_codes)
    |> ErrorParsers.parse_if_error()
  end

  @doc """
  Throws a no permission error for multiple permission codes with tenant.

  Calls `auth.throw_no_permission`.
  Uses the (user_id, perm_codes, tenant_id) variant with a list.
  """
  @spec throw_no_permission_list(integer(), list(String.t()), integer()) :: {:error, any()}
  def throw_no_permission_list(user_id, perm_codes, tenant_id) when is_list(perm_codes) do
    db_context().auth_throw_no_permission(user_id, perm_codes, tenant_id)
    |> ErrorParsers.parse_if_error()
  end
end
